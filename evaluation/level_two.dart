...
import 'package:first_app/evaluation/level_three.dart';
import 'package:first_app/navbar/navigator_page.dart';
import 'package:flutter/material.dart';

class LevelTwo extends StatefulWidget {
  @override
  _LevelTwo createState() =>
      _LevelTwo();
}

class _LevelTwo
    extends State<LevelTwo> {
  ...
  void submit() async {
    ...
    if (user != null) {
      await FirebaseFirestore.instance.collection('users').doc(user.uid).update(
        {"mark": score, "stage_system": stage},
      );
    }
   
    if (score > 24) {// ---------True---------->Retest-------------
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => LevelThree()),
      );
    } else {//-------------false---------homepage-+-Upgrade stage----------
      SnackBar(
        content: Text("Your score: $score → Stage: $stage"),
        backgroundColor: Colors.green,
      );
